import { Component, OnInit } from '@angular/core';
import { ActivatedRoute, Router } from '@angular/router';
import { ApiService } from '../api.service';
import { NavparamService } from '../navparam.service';

@Component({
  selector: 'app-summary-reservation',
  templateUrl: './summary-reservation.page.html',
  styleUrls: ['./summary-reservation.page.scss'],
})
export class SummaryReservationPage  implements OnInit {
  carId:any;
  userId: any;
  data:any;
  total:any;
  price: any;
  days: any;
  address: any;
  dateTime: any;

id: any;
markOfTheCar: any;
modelOfTheCar: any;
carMade: any;
carCrossed: any;
carFuelType: any;
carCubicCapacity: any;
carEnginePower: any;
carTransmission: any;
carEmissionClass: any;
carClimate: any;
carDoors: any;
carBody: any;
carSeats: any;
carColor: any;
carDrive: any;
carPrice: any;
carType: any;
carImage: any;
accessories: any;
carLikes: any;
carDislikes:any;
carReserved:any;
carOwnerId: any;

  constructor(private activatedRoute:ActivatedRoute, private navService: NavparamService, private _apiService:ApiService,private router: Router) {
    this.data = this.navService.getNavData();
    this.carId = this.data.carId;
    this.address = this.data.takeoverAddress;
    this.dateTime = this.data.takeoverDate;
    this.userId = this.navService.getLoginUser();
    this.price = this.data.carPrice;
    this.days = this.data.numberOfDays;
    this.total = parseFloat(this.price) * parseFloat(this.days);
   }

  ngOnInit() {
  }

  makeReservation(){
    this.getCar(this.carId);
    console.log(this.markOfTheCar);

  }

  getCar(id){
    this._apiService.getCar(id).subscribe((res:any) =>{
      console.log("SUCCESS",res);
      let car = res[0];
      this.id = car.id;
      this.markOfTheCar = car.markOfTheCar;
      this.modelOfTheCar = car.modelOfTheCar;
      this.carMade = car.carMade;
      this.carCrossed = car.carCrossed;
      this.carFuelType = car.carFuelType;
      this.carCubicCapacity = car.carCubicCapacity;
      this.carEnginePower = car.carEnginePower;
      this.carTransmission = car.carTransmission;
      this.carEmissionClass = car.carEmissionClass;
      this.carClimate = car.carClimate;
      this.carDoors = car.carDoors;
      this.carBody = car.carBody;
      this.carSeats = car.carSeats;
      this.carColor = car.carColor;
      this.carDrive = car.carDrive;
      this.carPrice = car.carPrice;
      this.carType = car.carType;
      this.carImage = car.carImage;
      this.accessories = car.accessories;
      this.carLikes = car.carLikes;
      this.carDislikes = car.dislikes;
      this.carReserved = car.carReserved;
      this.carOwnerId = car.carOwnerId;

       console.log(this.markOfTheCar);
       this.updateCar();
       this.addReservation();
    },(err:any)=>{
      console.log("ERROR",err)
    });
  }



   updateCar(){
    let data = {
          markOfTheCar: this.markOfTheCar,
          modelOfTheCar: this.modelOfTheCar,
          carMade: this.carMade,
          carCrossed: this.carCrossed,
          carFuelType: this.carFuelType,
          carCubicCapacity: this.carCubicCapacity,
          carEnginePower: this.carEnginePower,
          carTransmission: this.carTransmission,
          carEmissionClass: this.carEmissionClass,
          carClimate: this.carClimate,
          carDoors: this.carDoors,
          carBody: this.carBody,
          carSeats: this.carSeats,
          carColor: this.carColor,
          carDrive: this.carDrive,
          carPrice: this.carPrice,
          carType: this.carType,
          carImage: this.carImage,
          accessories: this.accessories,
          carLikes: this.carLikes,
          carDislikes: this.carDislikes,
          carReserved: "true",
          carOwnerId: this.carOwnerId
    }


    this._apiService.updateCar(this.id,data).subscribe((res:any) =>{
      console.log("SUCCESS",res);
    },(err:any)=>{
      console.log("ERROR",err)
    });

  }



   addReservation(){

    let data = {
      takeoverDate: this.dateTime,
      takeoverAddress: this.address,
      numberOfDays: this.days,
      total: this.total,
      carId: this.carId,
      userId: this.userId,
      carImage: this.carImage
    }

    this._apiService.addReservation(data).subscribe((res:any)=>{
    console.log("SUCCESS ===",res);
      alert('New reservation is successfully created! ');
      this.router.navigateByUrl('/home');

    }, (error:any) =>{
      alert('ERROR');
      console.log("ERROR ===",error);
    })

  }

}
